...
def _seasonal_as_source(state: TradingState) -> dict:
    """Convert seasonal agent output to standard source format."""
    sig = state.get("seasonal_signal")
    if not sig:
        return {}
    return {
        "signal": sig,
        "confidence": state.get("seasonal_confidence", 0) / 100,
        "available": True,
    }
